...
struct YaraRuleStats {
    size_t totalRulesLoaded = 0;
    size_t filesScanned = 0;
    size_t matchesFound = 0;
    std::chrono::milliseconds averageScanTime{0};
    std::unordered_map<std::string, size_t> ruleMatchCounts;
    std::chrono::system_clock::time_point lastRuleUpdate;
};
...
private:
    ...
    YaraRuleStats m_stats;
    std::chrono::system_clock::time_point m_lastScanStart;
...
public:
    ...
    std::error_code reloadRules();
    std::error_code validateRules(const std::string& rulesPath);
    std::vector<std::string> getLoadedRuleNames() const;

    // Statistics methods
    YaraRuleStats getStatistics() const;
    void resetStatistics();
    
    // Configuration methods
    void setMaxFileSize(size_t maxSize);
    void setScanTimeout(std::chrono::seconds timeout);

private:
    void updateScanStatistics(const std::vector<std::string>& matches);
    std::error_code loadRuleFromString(const std::string& ruleContent, const std::string& ruleName);
};
